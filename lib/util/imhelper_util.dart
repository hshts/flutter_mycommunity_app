...
class ImHelper {
  final TableHelper _sql = TableHelper();
    ...
    if (grouprelations.isNotEmpty) {
      for (GroupRelation groupRelation in grouprelations) {
        ...
        if (await isJoinGrid(groupRelation.uid!, groupRelation.timeline_id)) {
          int? unreadcount = Sqflite.firstIntValue(
            await dbClient.rawQuery(
              "SELECT unreadcount FROM ${TableHelper.im_group_relation} "
          ...
          await dbClient.rawUpdate(
            'UPDATE ${TableHelper.im_group_relation} SET unreadcount = ?, readindex = ?, group_name1 = ?,clubicon=?,'
            'name =?,newmsgtime=?,newmsg=?,timelineType=?,status=?,locked=?,memberupdatetime=?,source_id=?,goodpriceid=? WHERE timeline_id = ? and uid = ? ',
            [
              unreadcount + groupRelation.unreadcount,
              groupRelation.readindex,
              groupRelation.group_name1,
              groupRelation.clubicon,
              groupRelation.name,
              groupRelation.newmsgtime,
              groupRelation.newmsg,
              groupRelation.timelineType,
              groupRelation.status,
              groupRelation.locked,
              groupRelation.memberupdatetime,
              groupRelation.source_id,
              groupRelation.goodpriceid,
  ...
  Future<int> saveGroupRelationOrderExpiration(
    ...
    if (grouprelations != null && grouprelations.isNotEmpty) {
      for (GroupRelation groupRelation in grouprelations) {
        timelineIds.add(groupRelation.timeline_id);
      }
      ...
      if (localGrouprelations != null && localGrouprelations.length > 0) {
        for (GroupRelation local in localGrouprelations) {
          if (!timelineIds.contains(local.timeline_id)) {
            await dbClient.rawUpdate(
              'UPDATE ${TableHelper.im_group_relation} SET isnotservice=1 WHERE timeline_id = ? and uid = ?',
  ...
  Future<void> updateGroupRelation(
    ...
    await dbClient.rawUpdate(
      'UPDATE ${TableHelper.im_group_relation} SET oldmemberupdatetime = ? WHERE timeline_id = ? ',
      [oldupdatetime, timelineId],
    );
  }
  ...
  //更新relation
  Future<void> updateGroupRelationIsNotService(String timelineId) async {
    var dbClient = await _sql.db;

    await dbClient.rawUpdate(
      'UPDATE ${TableHelper.im_group_relation} SET isnotservice = 0 WHERE  uid = ? and timeline_id = ? ',
      [Global.profile.user!.uid, timelineId],
    );
  }

  Future<List<User>?> getGroupMemberRelation(String timelineId) async {
    var dbClient = await _sql.db;
    List<Map> maps = await dbClient.rawQuery(
      "SELECT * FROM ${TableHelper.im_groupandcommunity_member_relation} "
  ...
  ///保存安全活动规范
  Future<void> saveInitMessage(String timelineId) async {
    var dbClient = await _sql.db;
    var tem = Sqflite.firstIntValue(
      await dbClient.rawQuery(
        "SELECT COUNT(*) FROM ${TableHelper.im_timeline_sync_relation} "
        "WHERE uid=${Global.profile.user!.uid} and timeline_id='$timelineId' and content='@安全活动规范@'",
      ),
    );
  ...
  ///撤回message
  Future<int> recallMessage(String sourceId) async {
    var dbClient = await _sql.db;
    var result = 0;
  ...
  ///撤回message
  Future<int> recallMessageToUid(String sourceId, String content) async {
    var dbClient = await _sql.db;
    var result = 0;
  ...
  ///撤回im_group_relation
  Future<int> recallGroupRelation(String sourceId, String content) async {
    var dbClient = await _sql.db;
    var result = 0;
  ...
  Future<int> getMaxsequence_id(String timelineId) async {
    var dbClient = await _sql.db;
    int? result = Sqflite.firstIntValue(
      await dbClient.rawQuery(
        "SELECT sequence_id FROM im_timeline_sync_relation WHERE timeline_id=? "
        "and uid=${Global.profile.user!.uid} "
  ...
  Future<List<GroupRelation>> getGroupRelation(String uid) async {
    var dbClient = await _sql.db;
    List<Map> maps = await dbClient.rawQuery(
      "SELECT * FROM ${TableHelper.im_group_relation} where  uid=$uid and isdel=0 order by istop desc,newmsgtime desc",
    );
    if (maps.isEmpty) {
      return [];
    }
  ...
  //当前用户是否加入群聊
  Future<bool> isJoinGrid(int uid, String timelineId) async {
    var dbClient = await _sql.db;

    int? result = Sqflite.firstIntValue(
      await dbClient.rawQuery(
        "SELECT COUNT(*) FROM ${TableHelper.im_group_relation} WHERE  timeline_id='$timelineId'  "
        "and uid=${Global.profile.user!.uid} ",
      ),
    );
    result ??= 0;
  ...
  //更新群关系中未读数量，变成0
  Future<int> updateAlready(String timelineId) async {
    var dbClient = await _sql.db;
    int readindex = await getMaxsequence_id(timelineId);
    int count = await dbClient.rawUpdate(
      'UPDATE ${TableHelper.im_group_relation} SET unreadcount = ?, readindex = ? WHERE timeline_id = ? and uid=?',
      [0, readindex, timelineId, Global.profile.user!.uid],
    );
  ...
  Future<int> updateRelationStatus(String timelineId, int status) async {
    var dbClient = await _sql.db;
    int count = await dbClient.rawUpdate(
      'UPDATE ${TableHelper.im_group_relation} SET status = ? WHERE timeline_id = ? and uid=?',
      [status, timelineId, Global.profile.user!.uid],
    );
  ...
  //将指定群聊置顶
  Future<int> updateTop(String timelineId, int uid) async {
    var dbClient = await _sql.db;
    int count = await dbClient.rawUpdate(
  ...
  //取消群置顶
  Future<int> updateTopCancel(String timelineId, int uid) async {
    var dbClient = await _sql.db;
    int count = await dbClient.rawUpdate(
      'UPDATE ${TableHelper.im_group_relation} SET istop = ? WHERE timeline_id = ? and uid=?',
      [0, timelineId, uid],
  ...
  //删除群聊
  Future<int> delGroupRelation(String timelineId, int uid) async {
    var dbClient = await _sql.db;
    int count = await dbClient.rawUpdate(
  ...
  Future<void> delTimeLineSync(int uid, String timelineId) async {
    var dbClient = await _sql.db;
    await dbClient.rawQuery(
      "delete FROM ${TableHelper.im_timeline_sync_relation} where  uid=$uid and timeline_id='$timelineId' ",
    );
  }
  ...
  Future<List<CommentReply>?> getCommentReplys(int current, int offset) async {
    var dbClient = await _sql.db;
    List<Map> maps = await dbClient.rawQuery(
      "SELECT * FROM ${TableHelper.t_Comment_Reply} where  "
  ...
  Future<List<Follow>> getFollows(int current, int offset) async {
    var dbClient = await _sql.db;
    List<Map> maps = await dbClient.rawQuery(
      "SELECT * FROM ${TableHelper.t_Follow} where  "
  ...
  //更新红包已经打开
  Future<int> updateReceiveRedPacket(String content, String timelineId) async {
    var dbClient = await _sql.db;
    var result = 0;
    result = await dbClient.rawUpdate(
      'UPDATE ${TableHelper.im_timeline_sync_relation} SET isopen = ? WHERE timeline_id = ? and content = ? and uid=?',
      [1, timelineId, content, Global.profile.user!.uid],
    );
    if (Global.isInDebugMode) {
      print(result);
    }
  ...
  Future<int> delActivityState(String actid, int uid) async {
    ...
    var result = 0;
    result = await dbClient.rawUpdate(
      "delete from ${TableHelper.activity_state_table} "
      " WHERE actid = ? and uid= ?",
      [actid, uid],
    );
    print(result);
  ...
  Future<int> selActivityState(String actid, int uid, Function fun) async {
    var dbClient = await _sql.db;
    List<Map> maps = await dbClient.rawQuery(
      "SELECT * FROM ${TableHelper.activity_state_table}"
  ...
  Future<int> delGoodPriceState(String goodpriceid, int uid, int status) async {
    ...
    var result = 0;
    result = await dbClient.rawUpdate(
      "delete from ${TableHelper.goodprice_state_table} "
      " WHERE goodpriceid = ? and uid= ? and status = ?",
      [goodpriceid, uid, status],
    );
    return result;
  }
  ...
  Future<bool> selGoodPriceState(
    ...
    var dbClient = await _sql.db;
    List<Map> maps = await dbClient.rawQuery(
      "SELECT * FROM ${TableHelper.goodprice_state_table}"
  ...
  Future<int> delBugSuggestState(String actid, int uid, int type) async {
    ...
    var result = 0;
    result = await dbClient.rawUpdate(
      "delete from ${TableHelper.bugsuggest_state_table} "
      " WHERE actid = ? and uid= ? and type = ?",
      [actid, uid, type],
    );
    print(result);
    return result;
  ...
  Future<int> selBugSuggestStateCount(int uid, int type) async {
    ...
    var tem = Sqflite.firstIntValue(
      await dbClient.rawQuery(
        "SELECT count(*) FROM ${TableHelper.bugsuggest_state_table}"
  ...
  Future<int> selActivityStateCount(int uid) async {
    ...
    var tem = Sqflite.firstIntValue(
      await dbClient.rawQuery(
        "SELECT count(*) FROM ${TableHelper.activity_state_table}"
        " where  uid=$uid",
      ),
    );
    if (tem == null || tem == 0) {
  ...
  Future<int> selActivityCollectionCount(int uid) async {
    ...
    var tem = Sqflite.firstIntValue(
      await dbClient.rawQuery(
        "SELECT count(*) FROM ${TableHelper.activity_collection_state_table}"
        " where  localuid=$uid",
      ),
    );
    if (tem == null || tem == 0) {
  ...
  Future<int> delActivityCollectionState(String actid, int uid) async {
    ...
    var result = 0;
    result = await dbClient.rawUpdate(
      "delete from ${TableHelper.activity_collection_state_table} "
      " WHERE actid = ? and localuid= ?",
      [actid, uid],
    );
    print(result);
    return result;
  ...
  Future<int> delProductCollectionState(int productid, int uid) async {
    ...
    var result = 0;
    result = await dbClient.rawUpdate(
      "delete from ${TableHelper.product_collection_state_table} "
  ...
  Future<int> delGoodPriceCollectionState(String goodpriceid, int uid) async {
    ...
    var result = 0;
    result = await dbClient.rawUpdate(
      "delete from ${TableHelper.goodprice_collection_state_table} "
      " WHERE goodpriceid = ? and localuid= ?",
      [goodpriceid, uid],
    );
    if (Global.isInDebugMode) {
      print(result);
  ...
  Future<int> selActivityCommentCountState(int uid) async {
    ...
    var tem = Sqflite.firstIntValue(
      await dbClient.rawQuery(
        "SELECT count(*) FROM ${TableHelper.activity_comment_state_table}"
        " where  uid=$uid ",
      ),
    );
    if (tem == null || tem == 0) {
  ...
  Future<int> delActivityCommentState(String commentid, int uid) async {
    ...
    var result = 0;
    result = await dbClient.rawUpdate(
      "delete from ${TableHelper.activity_comment_state_table} "
  ...
  Future<int> selGoodPriceCommentCountState(int uid) async {
    ...
    var tem = Sqflite.firstIntValue(
      await dbClient.rawQuery(
        "SELECT count(*) FROM ${TableHelper.goodprice_comment_state_table}"
        " where  uid=$uid ",
      ),
    );
    if (tem == null || tem == 0) {
  ...
  Future<int> delGoodPriceCommentState(String commentid, int uid) async {
    ...
    var result = 0;
    result = await dbClient.rawUpdate(
      "delete from ${TableHelper.goodprice_comment_state_table} "
      " WHERE commentid = ? and uid= ?",
      [commentid, uid],
    );
    if (Global.isInDebugMode) {
  ...
  Future<int> selBugAndSuggestCommentCountState(int uid, int type) async {
    ...
    var tem = Sqflite.firstIntValue(
      await dbClient.rawQuery(
        "SELECT count(*) FROM ${TableHelper.activity_bugsuggestcomment_state_table}"
        " where  uid=$uid and type=$type",
      ),
    );
    if (tem == null || tem == 0) {
  ...
  Future<int> delBugAndSuggestCommentState(
    ...
    var result = 0;
    result = await dbClient.rawUpdate(
      "delete from ${TableHelper.activity_bugsuggestcomment_state_table} "
      " WHERE commentid = ? and uid= ? and type=?",
      [commentid, uid, type],
    );
    if (Global.isInDebugMode) {
      print('delete comment num $result');
  ...
  Future<int> selActivityEvaluateCountState(int uid) async {
    ...
    var tem = Sqflite.firstIntValue(
      await dbClient.rawQuery(
        "SELECT count(*) FROM ${TableHelper.activity_evaluate_state_table}"
        " where  uid=$uid ",
      ),
    );
    if (tem == null || tem == 0) {
  ...
  Future<int> delActivityEvaluateState(String v, int uid) async {
    ...
    var result = 0;
    result = await dbClient.rawUpdate(
      "delete from ${TableHelper.activity_evaluate_state_table} "
      " WHERE evaluateid = ? and uid= ?",
      [v, uid],
    );
    if (Global.isInDebugMode) {
  ...
  Future<int> saveReplys(
    ...
    for (CommentReply commentReply in commentreplys) {
      var tem = Sqflite.firstIntValue(
        await dbClient.rawQuery(
          "SELECT COUNT(*) FROM ${TableHelper.t_Comment_Reply} "
  ...
  Future<int> saveActivityLike(List<Like> likes) async {
    ...
    for (Like like in likes) {
      var tem = Sqflite.firstIntValue(
        await dbClient.rawQuery(
          "SELECT COUNT(*) FROM ${TableHelper.t_like} "
          "WHERE likeid=${like.likeid} and touid=${Global.profile.user!.uid} and liketype=${like.liketype}",
        ),
      );
      if (tem == 0)
        if (await dbClient.insert(TableHelper.t_like, like.toMap()) > 0) {
          result++;
        }
    }
  ...
  Future<int> getLikeNoticeCount() async {
    ...
    if (Global.profile.user != null) {
      int? result = Sqflite.firstIntValue(
        await dbClient.rawQuery(
          "SELECT count(*) FROM t_like WHERE  "
  ...
  Future<List<Like>> getThumbUps(int current, int offset) async {
    var dbClient = await _sql.db;
    List<Map> maps = await dbClient.rawQuery(
      "SELECT * FROM ${TableHelper.t_like} where  "
  ...
  Future<int> getMaxReplyid(ReplyMsgType type) async {
    ...
    int? result = Sqflite.firstIntValue(
      await dbClient.rawQuery(
        "SELECT replyid FROM Comment_Reply_table WHERE  "
  ...
  Future<int> getMaxLikeID(int type) async {
    ...
    int? result = Sqflite.firstIntValue(
      await dbClient.rawQuery(
        "SELECT likeid FROM t_like WHERE  "
  ...
  Future<int> saveFollow(List<Follow> follows) async {
    ...
    for (Follow follow in follows) {
      var tem = Sqflite.firstIntValue(
        await dbClient.rawQuery(
          "SELECT COUNT(*) FROM ${TableHelper.t_Follow} "
  ...
  Future<int> getNewFollowCount() async {
    ...
    if (Global.profile.user != null) {
      int? result = Sqflite.firstIntValue(
        await dbClient.rawQuery(
          "SELECT count(*) FROM t_Follow WHERE  "
  ...
  Future<int> getActivityEvaluateCount(int evaluatestatus) async {
    ...
    int? result = Sqflite.firstIntValue(
      await dbClient.rawQuery(
        "SELECT count(*) FROM unevaluate_activity WHERE   uid=${Global.profile.user!.uid} and actuid <> ${Global.profile.user!.uid} and evaluatestatus= $evaluatestatus",
      ),
    );
  ...
  Future<int> getSysNoticeCount(
    ...
    int? result = Sqflite.firstIntValue(
      await dbClient.rawQuery(
        "SELECT count(*) FROM Comment_Reply_table WHERE   touid=${Global.profile.user!.uid} and type='$replyMsgType' and isread=0 ",
      ),
    );
  ...
  Future<List<int>> selFollowState(int follow, int uid) async {
    var dbClient = await _sql.db;
    List<Map> maps = await dbClient.rawQuery(
      "SELECT * FROM ${TableHelper.community_follow_state_table}"
  ...
  Future<int> delFollowState(int follow, int uid) async {
    ...
    var result = 0;
    result = await dbClient.rawUpdate(
      "delete from ${TableHelper.community_follow_state_table} "
      " WHERE follow = ? and uid= ?",
      [follow, uid],
    );
    if (Global.isInDebugMode) {
  ...
  Future<int> getMaxMemberId() async {
    ...
    int? result = Sqflite.firstIntValue(
      await dbClient.rawQuery(
        "SELECT id FROM ${TableHelper.user_member_state_table} WHERE  "
  ...
  Future<int> getMaxFriendId() async {
    ...
    int? result = Sqflite.firstIntValue(
      await dbClient.rawQuery(
        "SELECT id FROM ${TableHelper.user_friend_state_table} WHERE  "
        " uid=${Global.profile.user!.uid} "
        "ORDER BY id desc LIMIT 1",
      ),
    );
  ...
  Future<int> getMaxUserSharedId() async {
    ...
    int? result = Sqflite.firstIntValue(
      await dbClient.rawQuery(
        "SELECT sharedid FROM ${TableHelper.user_shared_state_table}   "
  ...
  Future<int> getUserSharedCount() async {
    ...
    int? result = Sqflite.firstIntValue(
      await dbClient.rawQuery(
        "SELECT COUNT(*) FROM ${TableHelper.user_shared_state_table} "
        "WHERE uid=${Global.profile.user!.uid} and isread=0",
      ),
    );
    if (result != null) {
  ...
  Future<List<UserShared>?> getSharedFriend(int current, int offset) async {
    var dbClient = await _sql.db;
    List<Map> maps = await dbClient.rawQuery(
      "SELECT * FROM ${TableHelper.user_shared_state_table} where  "
      "uid=${Global.profile.user!.uid} order by createtime desc limit $current,$offset",
    );
    if (maps.isEmpty) {
      return null;
  ...
  Future<int> updateFriendMember(List<User> friends) async {
    ...
    for (User user in friends) {
      var tem = Sqflite.firstIntValue(
        await dbClient.rawQuery(
          "update  ${TableHelper.user_friend_state_table}  set username = '${user.username}',"
          " profilepicture='${user.profilepicture}'"
  ...
  Future<int> saveBrowseHistory(
    ...
    var result = 0;
    List<Map> maps = await dbClient.rawQuery(
      "SELECT * FROM ${TableHelper.history_browse_table}"
      " where  actid='$actid' and uid=${Global.profile.user!.uid} ",
    );
    if (maps.isEmpty) {
      result = await dbClient.insert(TableHelper.history_browse_table, {
        "actid": actid,
        "browsetime": CommonUtil.getTime(),
        "content": content,
        "coverimg": coverimg ?? '',
        "uid": Global.profile.user!.uid,
  ...
  Future<List<Activity>> getBrowseHistory(int current, int offset) async {
    var dbClient = await _sql.db;
    List<Map> maps = await dbClient.rawQuery(
      "SELECT * FROM ${TableHelper.history_browse_table} where  "
  ...
  Future<int> countBrowseHistory() async {
    ...
    int? result = Sqflite.firstIntValue(
      await dbClient.rawQuery(
        "SELECT count(*) FROM ${TableHelper.history_browse_table}"
        " where   uid=${Global.profile.user!.uid} ",
      ),
    );
    if (result == null) {
  ...
  Future<int> saveUnEvaluateActivity(
    ...
    result = await dbClient.rawUpdate(
      "delete from ${TableHelper.unevaluate_activity} "
      " WHERE  uid= ?",
      [uid],
    );
    ...
    for (int i = 0; i < activityEvaluates.length; i++) {
      ...
      result = await dbClient.insert(TableHelper.unevaluate_activity, {
        "actevaluateid": activityEvaluate.actevaluateid,
        "actid": activityEvaluate.activity!.actid,
        "createtime": activityEvaluate.activity!.createtime,
        "content": activityEvaluate.activity!.content,
        "coverimg": activityEvaluate.activity!.coverimg ?? '',
        "uid": Global.profile.user!.uid,
        "coverimgwh": activityEvaluate.activity!.coverimgwh,
        "profilepicture": activityEvaluate.activity!.user!.profilepicture,
        "username": activityEvaluate.activity!.user!.username,
        "evaluatestatus": activityEvaluate.evaluatestatus,
        "currentpeoplenum": activityEvaluate.activity!.currentpeoplenum,
  ...
  Future<int> getUnEvaluateActivityCount() async {
    ...
    if (Global.profile.user != null) {
      result = Sqflite.firstIntValue(
        await dbClient.rawQuery(
          "SELECT count(*) FROM ${TableHelper.unevaluate_activity} WHERE  "
          " uid=${Global.profile.user!.uid} ",
        ),
      );
    }
  ...
  Future<int> getMaxUnEvaluateActivityid() async {
    ...
    int? result = Sqflite.firstIntValue(
      await dbClient.rawQuery(
        "SELECT actevaluateid FROM ${TableHelper.unevaluate_activity} WHERE  "
        " uid=${Global.profile.user!.uid} "
  ...
  Future<int> updateUnEvaluateActivity(int actevaluateid) async {
    ...
    //先删除所有的活动,activity中的status是isevaluate的状态
    result = await dbClient.rawUpdate(
      'UPDATE ${TableHelper.unevaluate_activity} SET evaluatestatus = ? where actevaluateid=? and uid=? ',
  ...
  Future<int> updateUnEvaluateOrder() async {
    ...
    int result = 0;
    result = await dbClient.rawUpdate(
      'UPDATE ${TableHelper.user_orderunevaluate_state_table} SET orderunevaluatecount = '
  ...
  Future<int> updateUserOrder(int ordertype) async {
    ...
    if (ordertype >= 0) {
      result = await dbClient.rawUpdate(
        'UPDATE ${TableHelper.user_order_state_table} SET ordercount = ordercount-1 where ordertype=? and uid=? ',
  ...
  Future<int> saveNotInteresteduids(int uid, int notinteresteduid) async {
    ...
    result = await dbClient.insert(TableHelper.user_notinteresteduids, {
      "uid": uid,
      "notinteresteduid": notinteresteduid,
    });
  ...
  Future<int> delNotInteresteduids(int uid, int notinteresteduid) async {
    ...
    await dbClient.rawUpdate(
      'delete from ${TableHelper.user_notinteresteduids}  where uid = ? and notinteresteduid = ?',
      [uid, notinteresteduid],
    );
  ...
  Future<List<int>> getNotInteresteduids(int uid) async {
    ...
    List<Map> maps = await dbClient.rawQuery(
      "SELECT * FROM ${TableHelper.user_notinteresteduids} where  "
      "uid=$uid",
    );
    if (maps.isEmpty) {
      return list;
  ...
  Future<int> saveGoodPriceNotInteresteduids(
    ...
    result = await dbClient.insert(TableHelper.user_goodnotinteresteduids, {
      "uid": uid,
      "goodpricenotinteresteduid": goodpricenotinteresteduid,
    });
  ...
  Future<int> delGoodPriceNotInteresteduids(
    ...
    await dbClient.rawUpdate(
      'delete from ${TableHelper.user_goodnotinteresteduids}  where uid = ? and goodpricenotinteresteduid = ?',
      [uid, goodpricenotinteresteduid],
    );
  ...
  Future<List<int>> getGoodPriceNotInteresteduids(int uid) async {
    ...
    List<Map> maps = await dbClient.rawQuery(
      "SELECT * FROM ${TableHelper.user_goodnotinteresteduids} where  "
      "uid=$uid",
    );
    if (maps.isEmpty) {
      return list;
  ...
  Future<List<int>> getBlacklistUid(int uid) async {
    ...
    List<Map> maps = await dbClient.rawQuery(
      "SELECT * FROM ${TableHelper.user_blacklist} where  "
      "uid=$uid",
    );
    if (maps.isEmpty) {
      return list;
