...
class NetworkManager {
  static final ImBloc _imBloc = BlocProvider.of<ImBloc>(
    Global.mainkey.currentContext!,
  );
  ...
  static Future<void> onDone({bool isouted = false}) async {
    if (channel != null) {
  ...
  static void onMessage(data) {
    ...
    switch (type) {
      //有新消息
      case 'NEWIMMESSAGE':
        _imBloc.add(NewMessage(Global.profile.user!, content));
        break;
      //新的社团消息
      case 'NEWCOMMUNITYIMMESSAGE':
        _imBloc.add(NewCommunityMessage(Global.profile.user!, content));
        break;
      //新的私聊消息
      case 'NEWUSERIMMESSAGE':
        _imBloc.add(NewUserMessage(Global.profile.user!, content));
        break;
      //有新的回复
      case 'NEWREPLY':
        _imBloc.add(UserCommentReplyNotice(Global.profile.user!));
        break;
      //有新的评论
      case 'NEWCOMMENTY':
        _imBloc.add(UserCommentReplyNotice(Global.profile.user!));
      ...
      case 'RECALL':
        _imBloc.add(ReCallMessage(Global.profile.user!, content));
        break;
      //链接成功
      case 'success':
        /*
  ...
  static Future<void> initLogin() async {
    if (Global.profile.user != null) {
      ...
      _imBloc.add(
        UserRelationAndMessage(Global.profile.user!, userNotice: userNotice),
      );
    }
  }
