...
class ShareView extends StatefulWidget {
  ...
  final String? sharedtype; //分享类型 0 活动 1商品 2用户动态
  final String? actid;
  final int? createuid; //是谁发布的
  final Function? activityHomeLongPress;
  final Function? activityHomeOnTap;
  ...
  const ShareView({
    ...
    this.actid,
    this.createuid,
    this.activityHomeLongPress,
    this.activityHomeOnTap,
  });
...
class _ShareViewState extends State<ShareView> {
  ...
  Future<void> getNotInteresteduid() async {
    if (widget.sharedtype == "0" ||
        widget.sharedtype == "2" && Global.profile.user != null) {
      _notInterests ??= await _imHelper.getNotInteresteduids(
    ...
    if (widget.sharedtype == "1" && Global.profile.user != null) {
      if (_goodPriceNotInterests == null && Global.profile.user != null) {
        _goodPriceNotInterests = await _imHelper.getGoodPriceNotInteresteduids(
          Global.profile.user!.uid,
        );
      }
    }
  ...
  Future<void> showShareView() async {
    ...
    Widget gohomepage = Container(
      ...
      child: IconText(
        "去主页",
        icon: Icon(IconFont.icon_home_fill, color: Colors.orange, size: 30),
        ...
        onTap: () {
          ...
          } else if (widget.createuid != null &&
              widget.createuid != Global.profile.user!.uid) {
            Navigator.pushNamed(
              context,
              '/OtherProfile',
              arguments: {"uid": widget.createuid},
    ...
    Widget report = Container(
      ...
      child: _isMySelf
          ...
          : IconText(
              "举报",
              icon: Icon(IconFont.icon_jubao, color: Colors.red, size: 30),
              style: TextStyle(color: Colors.black87, fontSize: 13),
              direction: Axis.vertical,
              onTap: () {
                Navigator.pushNamed(
                  ...
                  arguments: {
                    "actid": widget.actid,
                    "sourcetype": int.parse(widget.sharedtype!),
                    "touid": widget.createuid,
                  },
                );
              },
            ),
          ...
          : IconText(
              ...
              onTap: () async {
                ...
                if (widget.sharedtype == "0" || widget.sharedtype == "2") {
                  if (_isActivityInterests) {
                    //设置不看
                    bool ret = await _userService.updateNotinteresteduids(
                    ...
                    if (ret) {
                      _imHelper.saveNotInteresteduids(
                        Global.profile.user!.uid,
                        widget.createuid!,
                      );
                    ...
                    if (ret) {
                      _imHelper.delNotInteresteduids(
                        Global.profile.user!.uid,
                        widget.createuid!,
                      );
                      setState(() {
                  ...
                  if (_isGoodPriceInterests) {
                    //设置不看
                    bool ret = await _userService.goodpricenotinteresteduids(
                      Global.profile.user!.token!,
                      Global.profile.user!.uid,
                      widget.createuid!,
                      (errcode, msg) {
                    ...
                    if (ret) {
                      _imHelper.saveGoodPriceNotInteresteduids(
                        Global.profile.user!.uid,
                        widget.createuid!,
                      );
                      //这里直接返回首页，首页在长按时弹出分享有点乱了。。
                    ...
                    if (ret) {
                      _imHelper.delGoodPriceNotInteresteduids(
                        Global.profile.user!.uid,
                        widget.createuid!,
                      );
                      setState(() {
                      ...
                      onTap: () {
                        Navigator.pushNamed(
                          ...
                          arguments: {
                            ...
                            "sharedtype": widget.sharedtype,
                            "image": img,
                            "localimg": widget.image,
                          },
                        );
                      },
                    ),
        ...
        return AlertDialog(
          ...
          actions: <Widget>[
            TextButton(
              child: Text('确定'),
              onPressed: () async {
                bool ret = await _imService.delMoment(
                  Global.profile.user!.token!,
                  Global.profile.user!.uid,
                  widget.contentid!,
