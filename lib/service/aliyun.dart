...
class AliyunService {
  //用户照片
  Future<SecurityToken?> getUserProfileSecurityToken(
    String token,
    int uid,
  ) async {
    ...
    await NetUtil.getInstance().post(
      formData,
      "/user/getUserProfileSecurityToken",
      (Map<String, dynamic> data) {
        securityToken = SecurityToken(
          data["data"]["ossAccessKeyId"],
          data["data"]["policy"],
          data["data"]["signature"],
          data["data"]["dir"],
          data["data"]["host"],
  ...
  Future<SecurityToken?> getActivitySecurityToken(String token, int uid) async {
    ...
    await NetUtil.getInstance().post(
      formData,
      "/Community/getActivitySecurityToken",
      (Map<String, dynamic> data) {
        securityToken = SecurityToken(
          data["data"]["ossAccessKeyId"],
          data["data"]["policy"],
  ...
  Future<SecurityToken?> getBugSecurityToken(String token, int uid) async {
    SecurityToken? securityToken;
    FormData formData = FormData.fromMap({"token": token, "uid": uid});
    await NetUtil.getInstance().post(formData, "/user/getBugSecurityToken", (
      Map<String, dynamic> data,
    ) {
      securityToken = SecurityToken(
        data["data"]["ossAccessKeyId"],
  ...
  Future<SecurityToken?> getMomentSecurityToken(String token, int uid) async {
    SecurityToken? securityToken;
    FormData formData = FormData.fromMap({"token": token, "uid": uid});
    await NetUtil.getInstance().post(formData, "/user/getMomentSecurityToken", (
      Map<String, dynamic> data,
    ) {
      securityToken = SecurityToken(
        data["data"]["ossAccessKeyId"],
  ...
  Future<SecurityToken?> getSoundSecurityToken(String token, int uid) async {
    SecurityToken? securityToken;
    FormData formData = FormData.fromMap({"token": token, "uid": uid});
    await NetUtil.getInstance().post(formData, "/IM/getSoundSecurityToken", (
      Map<String, dynamic> data,
    ) {
      securityToken = SecurityToken(
        data["data"]["ossAccessKeyId"],
  ...
  Future<String> uploadSound(
    ...
    FormData formData = FormData.fromMap({
      ...
      'file': await MultipartFile.fromFile(
        filePath,
        filename: filename,
        contentType: MediaType("audio", "mpeg4"),
      ), //必须放在参数最后
    });
    await NetUtil.aliyunOSSpost(formData, securityToken.host, (String data) {
      retUrl = soundUrl;
