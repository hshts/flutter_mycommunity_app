...
class _MyMessageState extends State<MyMessage> with TickerProviderStateMixin {
  ...
  bool get showCustomKeyBoard =>
      _activeEmojiGird || _activeMoreGrid || _activeSayingGrid;
  ...
  Future<void> stopRecorder() async {
    try {
      ...
      if (Global.isInDebugMode) print(_recordFilepath);
    } catch (err) {
  ...
  void initState() {
    ...
    if (_groupRelation.status != null) _relationStatus = _groupRelation.status!;
  ...
  Future<void> sendMessage(
    ...
    String serviceData = await _imService.postSendMessage(
      _groupRelation.timeline_id,
  ...
  Future<void> _blockUser(int status) async {
    ...
    if (status == 2) {
      ...
      ret = await _imService.updateCancelBlockUser(
        _groupRelation.timeline_id,
        Global.profile.user!.token!,
        Global.profile.user!.uid,
        _groupRelation.relationtype!,
      ...
      if (status == 2) {
        temLine.add(
          TimeLineSync(
            ...
            sendtime,
            user,
            _groupRelation.relationtype == 2 ? "你已将对方加入黑名单" : "你已屏蔽群消息",
            0,
            '',
            '',
          ),
        ...
        temLine.add(
          TimeLineSync(
            ...
            sendtime,
            user,
            _groupRelation.relationtype == 2 ? "已取消黑名单" : "你已取消屏蔽群消息",
            0,
            '',
            '',
          ),
  ...
  Future<void> _getGroupMember() async {
    ...
    if (_groupRelation.relationtype == 1) {
      List<User>? users = await _imService.getCommunityMemberList(
        _groupRelation.timeline_id,
        0,
      );
      if (users.isNotEmpty) {
    ...
    _imHelper.updateGroupRelation(
      _groupRelation.memberupdatetime!,
      _groupRelation.timeline_id,
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (MediaQuery.of(context).viewInsets.bottom > 0) {
      ...
      if (_initkeyboardHeight != MediaQuery.of(context).viewInsets.bottom) {
        _initkeyboardHeight = MediaQuery.of(context).viewInsets.bottom;
                ...
                child: InkWell(
                  ...
                  onTap: () {
                    Navigator.pushNamed(
                      context,
                      '/GoodPriceInfo',
                      arguments: {"goodprice": _goodprice},
                    ).then((value) {
                      FocusScope.of(context).requestFocus(FocusNode());
                    });
                  },
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
            ...
            _groupRelation.relationtype != 2
                ? IconButton(
                    ...
                    onPressed: () {
                      ...
                      if (_groupRelation.relationtype == 1) {
                        navurl = "/MemberList";
                        argMap = {
                          "cid": _groupRelation.timeline_id,
                          "status": _relationStatus,
                          "reporttype": _groupRelation.relationtype,
                        };
                      }
                ...
                : PopupMenuButton<String>(
                    onSelected: (String result) {
                      if (result == "去主页") {
                        String touid = _groupRelation.timeline_id.replaceAll(
          ...
          title: _groupRelation.group_name1 != null
              ? Text(
                  _groupRelation.group_name1!.length > 10
                      ? _groupRelation.group_name1!.substring(0, 10)
                      : _groupRelation.group_name1!,
      ...
      if (timeLineSyncs[i].sender == 0) {
        ...
        if (((i + 1 < timeLineSyncs.length &&
                DateTime.parse(timeLineSyncs[i].send_time!)
                        .difference(
                          DateTime.parse(timeLineSyncs[i + 1].send_time!),
                        )
                        .inMinutes
                        .abs() >
                    5) ||
                ...
                child: Container(
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 5),
                      Text(
                        '为了确保您的资金安全，请遵守出来玩吧交易规范，一定要在平台内完成支付。',
  ...
  Widget buildMyContent(TimeLineSync timeLineSync) {
    ...
    if (timeLineSync.contenttype == ContentType.Type_Sound.index) {
      String key = timeLineSync.content!
          .replaceAll('|sound: ', '')
          .replaceAll('|', '');
      ...
      widContent = SizedBox(
        ...
        child: Row(
          ...
          children: <Widget>[
            ...
            !timeLineSync.isplay!
                ? Icon(IconFont.icon_saying, color: Colors.white)
                : MySpinKitWave(
                    color: Colors.white,
      ...
      isImg = true;
      //不要写在组件里组件有bug
      List<String> locatoninfo = timeLineSync.content!
          .replaceAll('|location: ', '')
          .replaceAll('|', '')
          .split('#');
      ...
      List<String> sharedinfo = timeLineSync.content!
          .replaceAll('|shared: ', '')
          .replaceAll('|', '')
          .split('#');
            ...
            InkWell(
              ...
              child: ExtendedText(
                ...
                specialTextSpanBuilder: MySpecialTextSpanBuilder(
                  showAtBackground: false,
                  pagewidth: _pageWidth_2,
                  isopen: timeLineSync.isopen == 0 ? false : true,
                ),
              ),
            ),
          ],
      ...
      widContent = SizedBox(
        ...
        child: Row(
          ...
          children: <Widget>[
            ...
            !timeLineSync.isplay!
                ? Icon(IconFont.icon_saying, color: Colors.black87)
                : MySpinKitWave(
                    color: Colors.black87,
                    type: MySpinKitWaveType.center,
                    size: 24,
      ...
      widContent = WPopupMenu(
        ...
        onValueChanged: (int value) {
          ...
          if (value == 2) {
            ...
            if (widget.groupRelation.relationtype == 0 ||
                widget.groupRelation.relationtype == 3) {
              Navigator.pushNamed(
                context,
                '/ReportAllMessage',
                arguments: {
                  "sourcetype": 4,
                  "actid": widget.groupRelation.timeline_id.toString(),
                  "touid": timeLineSync.sender,
                  "content": timeLineSync.content,
                },
              );
            }
      ...
      children: <Widget>[
        Container(
          ...
          child: NoCacheCircleHeadImage(
            imageUrl: timeLineSync.serderpicture!,
            uid: timeLineSync.sender!,
          ),
        ),
        Expanded(
          flex: 8,
          child: Container(
            ...
            child: InkWell(
              child: Container(
                ...
                child: Padding(padding: EdgeInsets.all(10), child: widContent),
              ),
              onTap: () async {
                if (timeLineSync.isplay!) {
                  stopPlayer();
          ...
          onTap: () async {
            ...
            if (_groupRelation.relationtype == 0 ||
              ...
              Navigator.pushNamed(
                context,
                '/ActivityInfo',
                arguments: {"actid": _groupRelation.timeline_id},
              ).then((value) {
                FocusScope.of(context).requestFocus(FocusNode());
              });
            } else {
        ...
        GestureDetector(
          ...
          onTap: () async {
            ...
            Navigator.pushNamed(
              ...
              arguments: {
                "timeline_id": _groupRelation.timeline_id,
                "timeline_type": _groupRelation.relationtype,
              },
            ).then((value) {
              if (value != null &&
  ...
  Future<void> soundLoadAndPaly(TimeLineSync timeLineSync) async {
    File soundFile;
    if (Global.isInDebugMode) print(timeLineSync.contenttype);
    if (timeLineSync.contenttype == ContentType.Type_Sound.index) {
  ...
  Future<void> fileSave(TimeLineSync timeLineSync) async {
    String url = timeLineSync.content!
        .replaceAll('|sound: ', '')
        .replaceAll('|', '')
        .split('#')[1];
              ...
              child: InkWell(
                child: Column(
                  children: [
                    ...
                    Expanded(
                      child: ListView(children: buildMemberList(newArr)),
                    ),
                  ],
                ),
                onTap: () {
  ...
  Future<bool> _isQuitActivity() async {
    GroupRelation? temGroupRelation = await _imHelper.getGroupRelationByGroupid(
      Global.profile.user!.uid,
      _groupRelation.timeline_id,
    );
