...
class RelationList extends StatefulWidget {
  const RelationList({super.key});

  @override
  _RelationListState createState() => _RelationListState();
}

enum WhyFarther { im, addfriend }
...
class _RelationListState extends State<RelationList> {
  ...
  Widget UserRelationList(
                      ...
                      ? badges.Badge(
                          ...
                          badgeContent: Text(
                            sysMessage.newlithumbup_count > 99
                                ? '...'
                                : sysMessage.newlithumbup_count.toString(),
                      ...
                      ? badges.Badge(
                          ...
                          badgeContent: Text(
                            sysMessage.follow_count > 99
                                ? '...'
                                : sysMessage.follow_count.toString(),
                            style: TextStyle(fontSize: 10, color: Colors.white),
                          ),
                          child: _widgetFollow,
                        )
                      ...
                      ? badges.Badge(
                          ...
                          badgeContent: Text(
                            sysMessage.commentreply_count > 99
                                ? '...'
                                : sysMessage.commentreply_count.toString(),
                            style: TextStyle(fontSize: 10, color: Colors.white),
                          ),
                          child: _widgetCommentReply,
                        )
                      : _widgetCommentReply,
                  Container(
                    margin: EdgeInsets.only(top: 10),
    ...
    relations = ListView(children: lists);
  ...
  List<Widget> getRelationList(List<GroupRelation> groupRelations) {
    ...
    for (var item in groupRelations) {
      if (item.newmsg != null) {
            ...
            children: <Widget>[
              ...
              Stack(
                children: <Widget>[
                  SizedBox(
                    height: 45,
                    child: item.relationtype == 1
                        ? CommunityClipRRectHeadImage(
                            imageUrl: item.clubicon,
                            width: 45,
                            cid: item.timeline_id,
                          ...
                          child: Column(
                            ...
                            children: <Widget>[
                              Container(
                                margin: EdgeInsets.only(right: 10),
                                child: Text(
                                  item.newmsgtime != null &&
                                          item.newmsgtime!.isNotEmpty
                                      ? item.newmsgtime!.substring(10, 16)
                                      : "",
                                  style: TextStyle(
              ...
              items: <PopupMenuEntry>[
                PopupMenuItem(
                  child: GestureDetector(
                    ...
                    onTap: () {
                      if (item.istop == 0) {
                        _imBloc.add(
                          RelationTop(Global.profile.user!, item.timeline_id),
                        );
                      } else {
                        _imBloc.add(
                          RelationTopCancel(
                ...
                PopupMenuItem(
                  child: GestureDetector(
                    ...
                    onTap: () {
                      _imBloc.add(
                        RelationDel(Global.profile.user!, item.timeline_id),
                      );
                      Navigator.pop(context);
                    },
                  ),
  ...
  int getUidByTimeline_id(String timelineId) {
    int index = timelineId.indexOf(Global.profile.user!.uid.toString());
    String tem = "";
    if (index == 0) {
      tem = timelineId.substring(
        Global.profile.user!.uid.toString().length,
        timelineId.length,
      );
    } else {
      tem = timelineId.substring(0, Global.profile.user!.uid.toString().length);
    }
