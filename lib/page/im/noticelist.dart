...
class _NoticeListState extends State<NoticeList> {
  ...
  Future<void> getNoticelList() async {
    ...
    List<CommentReply>? temcommreplys = await imHelper.getCommentReplys(
      0,
      5000,
    );
    ...
    return Container(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return item.type == ReplyMsgType.sysnotice.toString()
              ? Padding(
                  padding: EdgeInsets.only(bottom: 10),
                  child: buildSysNotice(item),
                )
              : buildCommentReply(item);
        },
      ),
    );
  }
      ...
      child: Column(
        ...
        children: <Widget>[
          Container(
            ...
            child: Text(
              item.replycreatetime != null ? item.replycreatetime! : '',
  ...
  Widget buildCommentReply(CommentReply item) {
    String msgtype = "留言";
    if (item.type == ReplyMsgType.evaluatemsg.toString()) {
      //评价
      msgtype = "评价";
      ...
      children: <Widget>[
        GestureDetector(
          ...
          onTap: () async {
            ...
            if (item.type == ReplyMsgType.commentmsg.toString() ||
                item.type == ReplyMsgType.replymsg.toString()) {
              Navigator.pushNamed(
                context,
                '/ActivityInfo',
                arguments: {
              ...
              GoodPiceModel? goodprice = await gpservice.getGoodPriceInfo(
                item.actid!,
              );
              if (goodprice != null) {
                Navigator.pushNamed(
              ...
              EvaluateActivity? evaluateactivity = await activityService
                  .getEvaluateActivityByEvaluateid(item.evaluateid!);
              ...
              Navigator.pushNamed(
                context,
                '/SuggestInfo',
                arguments: {"suggestid": item.actid},
              );
              ...
              Navigator.pushNamed(
                ...
                item.type == ReplyMsgType.goodpricereplymsg.toString()) {
              GoodPiceModel? goodprice = await gpservice.getGoodPriceInfo(
                item.actid!,
              );
              if (goodprice != null) {
