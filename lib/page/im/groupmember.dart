...
class GroupMember extends StatefulWidget {
  ...
  GroupMember({super.key, this.arguments}) {
...
class _State extends State<GroupMember> {
  ...
  Future<void> getAllUsers() async {
    activity = await _activityService.getActivityMember(
      widget.timeline_id,
      errorCallBack,
    );
    if (activity != null) {
      ...
      body: _users.isEmpty
          ...
          : ListView(
              children: [
                Container(
                  color: Colors.white,
                  child: Wrap(children: buildMemberList()),
                ),
                SizedBox(height: 10),
                buildDelAndQuit(),
                activity!.user!.uid == Global.profile.user!.uid
                    ? SizedBox(height: 5)
                    : SizedBox.shrink(),
                activity!.user!.uid == Global.profile.user!.uid
                    ? buildDelMember()
        ...
        return AlertDialog(
          ...
          actions: <Widget>[
            TextButton(
              child: Text('确定'),
              onPressed: () async {
                bool ret = await _activityService.delQuiteActivity(
                  widget.timeline_id,
                ...
                if (ret) {
                  int ret = await _imHelper.delGroupRelation(
                    widget.timeline_id,
                    Global.profile.user!.uid,
                  );
