...
class RedPacketList extends StatefulWidget {
  ...
  RedPacketList({super.key, this.arguments}) {
    if (arguments != null) {
...
class _RedPacketListState extends State<RedPacketList> {
  ...
  Future<void> getRedPacketDetail() async {
    redPackDetails = await _imService.getRedPacketDetail(
      Global.profile.user!.uid,
      Global.profile.user!.token!,
      widget.redPacketModel!.redpacketid,
      (code, msg) {
        ShowMessage.showToast(msg);
      },
    );
    if (redPackDetails.isNotEmpty) {
      if (mounted) {
        setState(() {});
      }
    }
      ...
      body: Column(
        children: [
          ...
          Container(
            ...
            color: Colors.white,
            child: Text(
              '一个红包共${widget.redPacketModel!.amount}元',
              style: TextStyle(color: Colors.black54, fontSize: 13),
            ),
          ),
          MyDivider(),
          redPackDetails.isNotEmpty
              ? Expanded(child: buildRedPackList())
              : SizedBox.shrink(),
        ],
      ),
    );
    ...
    return Column(
      ...
      children: [
        NoCacheClipRRectOhterHeadImage(
          imageUrl: widget.redPacketModel!.profilepicture,
          width: 66,
          uid: widget.redPacketModel!.uid,
        ),
        SizedBox(height: 5),
        Text(
          '来自${widget.redPacketModel!.username}的红包',
          style: TextStyle(color: Colors.black87, fontSize: 16),
        ),
        SizedBox(height: 5),
        Text(
          widget.redPacketModel!.content,
  ...
  Widget buildRedPackList() {
    List<Widget> details = [];
    for (var element in redPackDetails) {
      details.add(
        Container(
          padding: EdgeInsets.all(10),
          color: Colors.white,
          child: Row(
            ...
            children: [
              Row(
                ...
                children: [
                  NoCacheClipRRectHeadImage(
                    imageUrl: element.profilepicture,
                    width: 39,
                    uid: element.uid,
                  ),
                  SizedBox(width: 5),
              ...
              Container(child: Text('${element.fund}元')),
            ],
          ),
        ),
      );
    ...
    return Container(
      color: Colors.white,
      child: ListView(children: details),
    );
  }
}
