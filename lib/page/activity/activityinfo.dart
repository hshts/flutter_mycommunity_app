...
class _ActivityState extends State<ActivityInfo> {
    ...
    return Scaffold(
      ...
      body: _activity != null
          ? SmartRefresher(
              ...
              child: ListView(
                // item 内容内边距
                children: <Widget>[actContent, actMember, actComment, actMore],
              ),
            )
          : promptWidget,
    ...
    return InkWell(
      onTap: () {
        Navigator.pushNamed(
          ...
          '/MapLocationShowNav',
          arguments: {
            "lat": _activity!.lat.toString(),
            "lng": _activity!.lng.toString(),
            "title": _activity!.addresstitle,
                ...
                children: <Widget>[
                  IconButton(
                    icon: Icon(
                      ...
                      color: _islike ? Global.profile.backColor : Colors.grey,
                    ),
                    onPressed: () {
                      if (!_islogin()) return;
  ...
  Future<void> updateLike() async {
    bool ret = await _activityService.updateLike(
      _activity!.actid,
      Global.profile.user!.uid,
  ...
  Future<void> updateDelLike() async {
    bool ret = await _activityService.delLike(
      _activity!.actid,
      Global.profile.user!.uid,
      Global.profile.user!.token!,
      errorCallBack,
  ...
  Future<void> updateCollection() async {
    bool ret = await _activityService.updateCollection(
      _activity!,
      Global.profile.user!.uid,
      Global.profile.user!.token!,
      errorCallBack,
  ...
  Future<void> updateDelCollection() async {
    bool ret = await _activityService.delCollection(
      _activity!.actid,
      Global.profile.user!.uid,
      Global.profile.user!.token!,
      errorCallBack,
  ...
  Future<void> joinGroupMessage() async {
    ...
    if (groupRelation != null) {
      ...
      groupRelation = await _activityService.getGroupConversation(
        _activity!.actid,
        Global.profile.user!.uid,
        Global.profile.user!.token!,
        errorCallBack,
  ...
  Future<void> updateCommentLike(commentid, int touid) async {
    bool ret = await _activityService.updateCommentLike(
      commentid,
      Global.profile.user!.uid,
      Global.profile.user!.token!,
      touid,
  ...
  Future<void> updateDelCommentLike(commentid, int touid) async {
    bool ret = await _activityService.delCommentLike(
      commentid,
      Global.profile.user!.uid,
      Global.profile.user!.token!,
      touid,
  ...
  Future<void> updatedelMessageReply(int replyid) async {
    bool ret = await _activityService.delMessageReply(
      Global.profile.user!.token!,
      Global.profile.user!.uid,
      replyid,
      _activity!.actid,
  ...
  Future<void> updateDelMessage(int commentid) async {
    bool ret = await _activityService.delMessage(
      Global.profile.user!.token!,
      Global.profile.user!.uid,
      commentid,
      _activity!.actid,
  ...
  Widget buildMoreActivity(List<Activity> activitys) {
    ...
    moreActivity = activitys.map((e) {
      return buildActivityItem(e);
    }).toList();
    int count = moreActivity.length % 2 == 0
...
class _UpdateActivityDialogState extends State<UpdateActivityDialog> {
  ...
  void initState() {
    super.initState();
    textcontent = widget.initialTextContent;
  }
              ...
              children: [
                ElevatedButton(
                  ...
                  onPressed: () async {
                    ...
                    if (ret) {
                      widget.activity.content = textcontent;
                    }
                    Navigator.pop(context);
                  },
                ),
