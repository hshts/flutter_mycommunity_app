...
class _GroupPurchaseState extends State<GroupPurchase> {
  ...
  void _getGoodPriceList() async {
    ...
    if (Global.profile.user != null) {
      goodpricenotinteresteduids = await _imHelper
            ...
            children: <Widget>[
              ...
              InkWell(
                child: Row(
                  ...
                  children: <Widget>[
                    Text(
                      Global.profile.locationGoodPriceName.length > 3
                          ? Global.profile.locationGoodPriceName.substring(0, 3)
                  ...
                  Navigator.pushNamed(
                    ...
                    if (value != null) {
                      if (Global.profile.locationGoodPriceCode !=
                          value["code"].toString()) {
                        Global.profile.locationGoodPriceCode = value["code"]
                            .toString();
                        Global.profile.locationGoodPriceName = value["name"]
                            .toString();
                        Global.saveProfile();
                        _getGoodPriceList();
                      }
  ...
  List<Widget> buildProductContent(List<GoodPiceModel> goodPiceModels) {
    ...
    for (int i = 0; i < goodPiceModels.length; i++) {
      ...
      Widget price = Row(
        ...
        children: [
          Text(
            '${goodPiceModel.mincost}—${goodPiceModel.maxcost}元',
            style: TextStyle(
              color: Colors.red,
              fontSize: 15,
              fontWeight: FontWeight.bold,
      ...
      widgets.add(
        ShareView(
          activityHomeLongPress: (bool isNotInterests) async {
            if (isNotInterests) {
              List<int> goodpricenotinteresteduids = await _imHelper
                  .getGoodPriceNotInteresteduids(Global.profile.user!.uid);
          ...
          activityHomeOnTap: () {
            Navigator.pushNamed(
              context,
              '/GoodPriceInfo',
              arguments: {"goodprice": goodPiceModel},
            );
          },
          sharedtype: "1",
                          ...
                          children: [
                            ...
                            Row(
                              children: [
                                ...
                                SizedBox(width: 10),
                                IconText(
                                  goodPiceModel.satisfactionrate == 0
                                      ? '0'
                                      : '${(goodPiceModel.satisfactionrate * 100).toInt()}%',
                                  padding: EdgeInsets.only(right: 2),
                                  style: TextStyle(
                                    color: Colors.black54,
                                    fontSize: 10,
