...
class _MyFollowState extends State<MyFollow> {
  final MyFollowBloc _myFollowBloc = MyFollowBloc();
  ...
  final double _categoryBarHeight = 180.0;
  final _loadstate = 0;
  ...
  final ScrollController _scrollControllerUserContent = ScrollController(
    initialScrollOffset: 0,
  );
  ...
  initState() {
    ...
    _scrollControllerUserContent.addListener(() {
      final maxScrollUser =
          _scrollControllerUserContent.position.maxScrollExtent;
    ...
    if (users.isNotEmpty) {
      for (int i = 0; i < users.length; i++) {
        if (i == activitys.length - 1) {
          _lockUser = false; //加载完毕后解锁，允许再次加载
        }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              users.isEmpty ? SizedBox(height: 10) : SizedBox(height: 5),
              users.isEmpty
                  ? SizedBox(
                      height: 26,
                      child: Center(
              ...
              users.isEmpty ? SizedBox(height: 10) : SizedBox(height: 0),
            ],
          ),
        ),
      ),
  ...
  Widget activityContent(List<Activity> activitys, PostSuccess state) {
    ...
    return StaggeredGridView.countBuilder(
      ...
      itemCount: state.hasReachedActivityMax
          ? activitys.length
          : activitys.length + 1,
      itemBuilder: (BuildContext context, int index) {
        ...
        return index > activitys.length
            ? buildLoading()
            : (index == activitys.length
                  ? SizedBox.shrink()
                  : buildActivityItem(activitys[index], _loadstate));
      },
