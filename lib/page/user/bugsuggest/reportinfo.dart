...
class MyReportInfo extends StatefulWidget {
  ...
  MyReportInfo({super.key, this.arguments}) {
...
class _MyReportState extends State<MyReportInfo> {
  ...
  Future<void> getReportInfo() async {
    myReport = await _activityService.getMyReportInfo(
    ...
    return Column(
      children: [
        ...
        Container(
          ...
          color: Colors.white,
          alignment: Alignment.center,
          child: myReport!.repleycontent != ""
              ? Text(
                  myReport!.repleycontent,
                  style: TextStyle(color: Colors.black54, fontSize: 14),
                )
              : Text(
                  '感谢您的反馈,我们会尽快核实处理，感谢您为净化出来玩吧社区环境作出的贡献。',
  ...
  Widget buildCenterContent() {
    ...
    if (myReport!.sourcetype == 0) {
      contentImg =
          myReport!.activity!.coverimg != null &&
              myReport!.activity!.coverimg != ""
          ? ClipRRectOhterHeadImage(
              imageUrl: myReport!.activity!.coverimg!,
              width: 100,
            )
          : SizedBox.shrink();
      content = Expanded(
        child: Text(
          myReport!.activity!.content,
    ...
    if (myReport!.sourcetype == 1) {
      contentImg = myReport!.goodPiceModel!.pic != ""
          ? ClipRRectOhterHeadImage(
              imageUrl: myReport!.goodPiceModel!.pic,
              width: 100,
            )
          : SizedBox.shrink();
      content = Expanded(
        child: Text(
          myReport!.goodPiceModel!.title,
          style: TextStyle(color: Colors.black54, fontSize: 12),
          overflow: TextOverflow.ellipsis,
          maxLines: 3,
        ),
    ...
    if (myReport!.sourcetype == 2 ||
        myReport!.sourcetype == 3 ||
        myReport!.sourcetype == 4 ||
        myReport!.sourcetype == 5 ||
        myReport!.sourcetype == 6 ||
        myReport!.sourcetype == 7 ||
        myReport!.sourcetype == 8) {
      contentImg = SizedBox.shrink();
      contentImg =
          myReport!.user!.profilepicture != null &&
              myReport!.user!.profilepicture! != ""
          ? ClipRRectOhterHeadImage(
              imageUrl: myReport!.user!.profilepicture!,
              width: 100,
            )
          : SizedBox.shrink();
      content = Expanded(
        child: Text(
          myReport!.user!.username,
          style: TextStyle(color: Colors.black54, fontSize: 12),
          overflow: TextOverflow.ellipsis,
          maxLines: 3,
        ),
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.start,
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [contentImg, SizedBox(width: 10), content],
          ),
          SizedBox(height: 10),
          MyDivider(),
          SizedBox(height: 10),
