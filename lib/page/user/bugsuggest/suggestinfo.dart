...
class _SuggestInfoState extends State<SuggestInfo> {
  Suggest? _suggest;
  ...
  Future<void> getSuggest() async {
    ...
    if (_suggest != null) {
      listComments = await imService.getSuggestCommentList(
        widget.suggestid,
        Global.profile.user!.uid,
        errorCallBack,
      );
      ...
      child: Row(
        children: <Widget>[
          NoCacheCircleHeadImage(
            width: 60,
            uid: _suggest!.user!.uid,
            imageUrl: _suggest!.user!.profilepicture!,
          ),
              ...
              children: <Widget>[
                ...
                Text(
                  CommonUtil.datetimeFormat(
                    DateTime.parse(_suggest!.createtime),
                  ),
                ),
              ],
            ),
  ...
  Widget buildContentImg() {
    ...
    if (Global.isInDebugMode) {
      print(
        '${_listimgs[0]}?x-oss-process=image/resize,m_fixed,w_1080/sharpen,50/quality,q_80',
      );
    }
    return Column(
            ...
            children: <Widget>[
              Column(
                ...
                children: <Widget>[
                  IconButton(
                    ...
                    onPressed: () async {
                      if (_isLikeEnter) {
                        _isLikeEnter = false;
                        if (!_islike) {
                          if (await imService.updateSuggestLike(
                            widget.suggestid,
                            Global.profile.user!.uid,
                            Global.profile.user!.token!,
                            errorCallBack,
                            ...
                            setState(() {
                              _suggest!.likenum = _suggest!.likenum + 1;
                              _islike = true;
                            });
                          }
                        } else {
                          if (await imService.delSuggestLike(
                            widget.suggestid,
                            Global.profile.user!.uid,
                            Global.profile.user!.token!,
                            errorCallBack,
                  ...
                  Text(
                    _suggest == null ? "0" : _suggest!.likenum.toString(),
                    style: TextStyle(
                      color: Colors.black54,
                      fontSize: 14,
                  ...
                  IconButton(
                    ...
                    onPressed: () {
                      if (Global.profile.user != null) {
                        ...
                        sendMessage(0, _suggest!.user!.uid);
                      } else {
                ...
                TextButton(
                  ...
                  onPressed: () async {
                    if (_message.isNotEmpty) {
                      if (commentid == 0) {
                        Navigator.pop(context);
                        int retcommentid = await imService.updateSuggestMessage(
                          widget.suggestid,
                          Global.profile.user!.uid,
                          Global.profile.user!.token!,
                          touid,
        ...
        return BlockPuzzleCaptchaPage(
          onSuccess: (v) async {
            if (commentid == 0) {
              ...
              int temreplyid = await imService.updateSuggestCommentReply(
                commentid,
                widget.suggestid,
                Global.profile.user!.uid,
                Global.profile.user!.token!,
