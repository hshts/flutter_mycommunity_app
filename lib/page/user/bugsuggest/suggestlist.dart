...
class _SuggestListState extends State<SuggestList> {
  ...
  void _getBugList() async {
    suggests = await _imService.getSuggestList(
  ...
  void _onLoading() async {
    if (!_ismore) return;
    final moredata = await _imService.getSuggestList(
      Global.profile.user!.uid,
      Global.profile.user!.token!,
      suggests.length,
      errorResponse,
            ...
            child: InkWell(
              onTap: () {
                Navigator.pushNamed(
                  context,
                  '/SuggestInfo',
                  arguments: {"suggestid": e.suggestid},
                );
              },
              child: Column(
                ...
                children: [
                  Row(
                    children: [
                      NoCacheCircleHeadImage(
                        imageUrl: e.user!.profilepicture!,
                        uid: e.user!.uid,
                      ),
                      SizedBox(width: 10),
                      Column(
                  ...
                  SizedBox(height: 10),
                  Container(child: Text(e.content)),

                  Row(
                    mainAxisAlignment: MainAxisAlignment.end,
                    children: [
                      IconText(
                        e.likenum.toString() == "0"
                            ? '点赞'
                            : e.likenum.toString(),
                      ...
                      SizedBox(width: 20),
                      IconText(
                        e.commentcount.toString() == "0"
                            ? '评论'
                            : e.commentcount.toString(),
                        padding: EdgeInsets.only(right: 2),
                        style: TextStyle(color: Colors.black54, fontSize: 13),
                        icon: Icon(
