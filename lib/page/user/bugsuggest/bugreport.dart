...
class _BugReportState extends State<BugReport> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title:
            Text('Bug内容', style: TextStyle(color: Colors.black, fontSize: 16)),
        centerTitle: true,
      ),
      body: Column(
        ...
        onPressed: () async {
          if (Global.profile.user != null) {
            if (_textEditingController.text == "") {
              ShowMessage.showToast("请填写BUG描述");
              return;
            }
            String temimages = "";
            ...
            String reportid = await _imService.reportBUG(
            ...
            if (reportid != null && reportid != "") {
              Navigator.pushReplacementNamed(context, '/BugInfo',
                  arguments: {"bugid": reportid});
            }
          } else {
  ...
  Future<void> loadAssets() async {
    ...
    if (resultList != null && resultList.length != 0) {
      ...
      _securityToken = await _aliyunService.getBugSecurityToken(
  ...
  loadingBlockPuzzle(BuildContext context,
      {barrierDismissible = true, String? temimages}) {
    showDialog<Null>(
      context: context,
