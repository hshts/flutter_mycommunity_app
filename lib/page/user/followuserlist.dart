...
class _MyFollowUserState extends State<MyFollowUser> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title:
            Text('关注的人', style: TextStyle(color: Colors.black, fontSize: 16)),
        centerTitle: true,
      ),
      body: SmartRefresher(
        enablePullDown: true,
  ...
  void _getFollowList() async {
    users =
        await _userService.getFollowUsersCommunity(Global.profile.user!.uid, 0);
  ...
  void _onLoading() async {
    ...
    final moredata = await _userService.getFollowUsersCommunity(
        Global.profile.user!.uid, users.length);
