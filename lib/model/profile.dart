...
class Profile {
  ...
  Profile({
    this.user,
    this.backColorval = 0xffff2442,
    this.fontColorval = 0xFFFFFFFF,
    this.locationName = "",
    this.locationCode = "",
    profilePicture = "",
    this.isLogGuided = true,
    this.lat = 0,
    this.lng = 0,
    this.locationGoodPriceCode = "",
    this.locationGoodPriceName = "",
  }) {
    if (user != null) {
      if (user!.profilepicture != null && user!.profilepicture!.isNotEmpty) {
        defProfilePicture = NetworkImage(user!.profilepicture!);
      }
    } else {
